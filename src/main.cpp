...
unsigned int nTargetSpacing = 1 * 77; // 77 seconds
unsigned int nStakeMinAge = 60 * 60 * 24 * 33; // 33 days
unsigned int nStakeMaxAge = 60 * 60 * 24 * 99; // 99 days
...
const string strMessageMagic = "AlexanderIIIMagnus Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AlexanderIIIMagnus", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Don’t dismiss bankers' predictions of a bitcoin bubble – they should know";
        CTransaction txNew;
        txNew.nTime = 1505649675;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1505649675;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2732425;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x2a29d566d737d6e5e0c2c29158bcded19a96b83e186ea5d65b0060447b9d991a"));
        block.print();
